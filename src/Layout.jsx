        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-3">
            {isMobile && isDetailPage ? (
              <Button variant="ghost" size="icon" onClick={() => navigate(-1)} className="text-[#8B4513] dark:text-amber-400 hover:bg-amber-100 dark:hover:bg-amber-900/20">
                <ArrowLeft className="w-6 h-6" />
              </Button>
            ) : (
              <Link to={createPageUrl("Announcements")} className="flex items-center gap-3 group">
                <img src="https://qtrypzzcjebvfcihiynt.supabase.co/storage/v1/object/public/base44-prod/public/68e4114143e84ad0df65d068/512622c87_1762982225481.jpg" alt="Bountiful Blessings Food Pantry" className="h-12 sm:h-16 w-auto object-contain transition-transform group-hover:scale-105" />
              </Link>
            )}

            <nav className="hidden lg:flex items-center gap-1">
              {[
                { label: t('nav.home'), path: "Announcements" },
                { label: t('nav.resources'), path: "Resources" },
                { label: t('nav.gallery'), path: "Gallery" },
              ].map(item => (
                <Link key={item.path} to={createPageUrl(item.path)} className={`px-3 py-2 rounded-xl font-medium transition-all text-sm ${location.pathname === createPageUrl(item.path) ? "bg-amber-200 dark:bg-amber-900 text-[#5C2E0F] dark:text-white" : "text-[#8B4513] dark:text-white hover:bg-amber-100 dark:hover:bg-amber-900/20"}`}>
                  {item.label}
                </Link>
              ))}
        ...
        <nav className="fixed bottom-0 left-0 right-0 bg-[#FFFBF0] dark:bg-card border-t border-amber-200 dark:border-amber-800 z-40 safe-area-bottom">
          <div className="flex justify-around items-center py-2 px-2">
            {bottomTabItems.map((item) => {
              ...
              return (
                <button key={item.path} onClick={() => { if (isActive) { window.scrollTo({ top: 0, behavior: 'smooth' }); navigate(createPageUrl(item.path), { replace: true }); } else { navigate(createPageUrl(item.path)); } }} className={`flex flex-col items-center gap-1 px-3 py-2 rounded-xl transition-all ${isActive ? "bg-amber-200 dark:bg-amber-900 text-[#5C2E0F] dark:text-white" : "text-[#8B4513] dark:text-white"}`}>
      ...
      <footer className="bg-[#FFFBF0] dark:bg-card backdrop-blur-sm border-t border-amber-200 dark:border-amber-800 mt-16 safe-area-bottom">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
            <img src="https://qtrypzzcjebvfcihiynt.supabase.co/storage/v1/object/public/base44-prod/public/68e4114143e84ad0df65d068/512622c87_1762982225481.jpg" alt="Bountiful Blessings Food Pantry" className="h-12 w-auto object-contain" />
            <p className="text-sm text-[#8B4513] dark:text-amber-200">{t('footer.serving')}</p>
          </div>
        </div>
      </footer>
    </div>
